...
word_cloud_df['content'] = word_cloud_df['content'].fillna('')
top_words = Counter(" ".join(word_cloud_df['content'].str.lower()).split()).most_common(200)
top_words = " ".join([word[0] for word in top_words if word[0] not in ["or","you’re","nagsimula","didnt","ka","me?","you?","tapos","ang","i'm","you're","had","☹️","it's","it’s","what","if","about","not","on","i’m","at","with","are","be","this","but","is","a","the","for","do","have","ng","na","me","so","in","of","and","an","you.","to","it","was","that","just","iyong","nag-react","nag-like","message","sa"]])
...
time_fig = px.imshow(time_heat, y=list(calendar.values()))
...
call_weekday_fig = px.imshow(call_weekday_heat, y=week)
...
st.write("##### We were most active at around", highlight(time_hour,"blue"),":blue[:00] on",highlight(calendar[time_month],"blue")," when we sent a total of", highlight(time_activity,"blue"), "messages to each other.")
st.write(time_fig)
...
calendar_bar = ["January","January","February","February","March","March","April","April","May","May","June","June","July","July","August","August","September","September","October","October","November","November","December","December"]
sender_df = df.groupby(['month', 'sender_name']).count().reset_index()
sender_df.drop(['year','timestamp_ms','day','hour','weekday','share','call_duration','sticker','length'], axis=1, inplace=True)

sender_content = sender_df.copy()
sender_content.drop(['reactions','photos','videos','audio_files'], axis=1, inplace=True)
...
sender_reactions.drop(['content','photos','videos','audio_files'], axis=1, inplace=True)
...
sender_photos.drop(['content','reactions','videos','audio_files'], axis=1, inplace=True)
...
sender_videos.drop(['content','reactions','photos','audio_files'], axis=1, inplace=True)
...
sender_audio_files.drop(['content','reactions','photos','videos'], axis=1, inplace=True)
